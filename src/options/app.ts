...
function render(settings: BetterXSettings): void {
  app.innerHTML = `
    ...
    <div class="settings-grid">
      ...
      <section class="panel"><div class="section-heading"><p class="eyebrow">PROFILE CONTEXT</p><h2>Profiles</h2><p>Use public profile signals only.</p></div><div class="check-list"><label><input id="countryEnabled" type="checkbox"><span><strong>Country flags</strong><small>Show a region badge when available.</small></span></label><label><input id="preferXRegion" type="checkbox"><span><strong>Prefer X account region</strong></span></label><label><input id="profileFallback" type="checkbox"><span><strong>Allow profile location fallback</strong></span></label></div></section>
  ...
  field<HTMLInputElement>("countryEnabled").checked = settings.countryFlags.enabled;
