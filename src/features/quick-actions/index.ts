...
export function injectQuickActions(post: ParsedPost, x: XDomAdapter, enabled: { block: boolean; notInterested: boolean; mute: boolean }, toast: (message: string) => void): void {
  ...
  add("Block", async () => { if (confirmAuthor("Block")) await (await x.openPostMenu(post)).choose("block"); }, enabled.block);
  ...
  add("Mute", async () => { if (confirmAuthor("Mute")) await (await x.openPostMenu(post)).choose("mute"); }, enabled.mute);
