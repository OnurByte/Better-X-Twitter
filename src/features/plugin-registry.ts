...
import { renderCountryBadge } from "./country-flags/ui";
...
export function contentPlugins(): BetterXPlugin[] {
  return [
    ...
    { id: "country-flags", name: "Country Flags", defaultEnabled: false, start(context) { const cache = new Map<string, ReturnType<typeof resolveCountry>>(); return context.settings.get().then((settings) => { if (!settings.features["country-flags"]) return; onPosts(context, (post) => { const handle = post.author.handle?.toLowerCase(); if (!handle || post.element.querySelector(".bx-country-badge")) return; const cached = cache.get(handle); if (cached) { post.element.append(renderCountryBadge(cached)); return; } void context.runtime.request({ type: "fx.profile", handle }).then((response: unknown) => { const result = response as { ok?: boolean; value?: { about_account?: { based_in?: string }; location?: string } }; if (!result.ok || !result.value) return; const country = resolveCountry(result.value, handle); cache.set(handle, country); if (country.countryCode) post.element.append(renderCountryBadge(country)); }).catch(() => {}); }); }); }, stop() {} },
